...
#SBATCH --mail-user=manuel.combarrosimon@uni.lu
#SBATCH --time=0-01:30:00      # 1 hour
#SBATCH --partition=batch
#__________________________
#SBATCH -N 6
#SBATCH --ntasks-per-node 16  # Optimized for 1 full node of aion
#SBATCH --cpus-per-task=8     # multithreading per task : -c --cpus-per-task <n> request
#__________________________
...
if [ -z "${TASKLISTFILE}" ]; then
    ...
    ${CMD_PREFIX} ${PARALLEL} --colsep ' ' -a ${TASKLISTFILE} "${ECHO} ${SRUN} ${TASK} {}"
fi
