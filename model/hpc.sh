...
do
  ...
  then
    ...
    srun --exclusive --cpu-bind=cores -n1 -c $cores python3 main.py --model_mzn ../mosaic_cloud2.mzn --dzn_dir ../data_sets --solver_name gecode --cp_timeout_sec $cp_timeout_sec --summary ../summary.csv --cp_strategy "free_search" --fzn_optimisation_level 1 $data_name 2>&1 | tee -a "$log_file" &
...
do
  ...
  then
    ...
    log_file=$res_dir"/"$cp_strategy"_"$uf_conflict_strategy"_"$uf_conflict_combinator"_"$algorithm"_"$cp_timeout_sec"_"$data_name
    echo "Start srun...."$log_file
    srun --exclusive --cpu-bind=cores -n1 -c $cores python3 main.py --model_mzn ../mosaic_cloud3.mzn --dzn_dir ../data_sets --solver_name gecode --cp_timeout_sec $cp_timeout_sec --summary ../summary.csv --cp_strategy "greedy" --fzn_optimisation_level 1 $data_name 2>&1 | tee -a "$log_file" &
    [[ $((tasks_counter % tasks)) -eq 0 ]] && wait && rm -f $summary".lock"
    let tasks_counter++
  fi
done
